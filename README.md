...
- **Hover tracking** — during layout only each phrase's on-screen rectangle is recorded (`phraseRects`) for hover hit-testing, with no suggestion-box DOM built. A `mousemove` handler hit-tests the pointer against those rects (plus the visible box's own rect, so the box stays open under the pointer) and builds the one suggestion box for the hovered phrase on demand, computing its geometry from that phrase's rect. The box overlays its phrase, sitting just above it (flipping below when there's no room); at most one box exists in the DOM at a time.
